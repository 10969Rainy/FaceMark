...
public class CameraManager : MonoBehaviour {
    ...
    public static float ScreenShootTexture2D = 0f;      //截取到图片编号(名称)
    public Text DebugScreenShootText;      //用于输出打印截图的名称(为了校验)
    ...
    {
        if (userInfoText.text == "" || groupIdText.text == "")
        {
            DebugScreenShootText.text = "请完整填写信息！";
            return;
        }
        OpenWebCamDevice();      //先打开摄像头
        Invoke("StartScreenShoot", 1.0f);      //1秒后截图
    }
    ...
    IEnumerator ScreenShoot()
    {
        ...
        if (!File.Exists(Application.dataPath + "/ScreenShoots/" + groupIdText.text))
        {
            DirectoryInfo dir = new DirectoryInfo(Application.dataPath + "/ScreenShoots/");
            dir.CreateSubdirectory(groupIdText.text);
        }
        File.WriteAllBytes(Application.dataPath + "/ScreenShoots/" + groupIdText.text + "/" + userInfoText.text + ".jpg", by);
        DebugScreenShootText.text = "截取到图片为:" + userInfoText.text.ToString();
        ScreenShootTexture2D += 1;
    }

    public void ToRegister()
    {
        SceneManager.LoadScene("02_Register");
    }
}
